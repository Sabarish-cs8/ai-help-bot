        ...
        return (
          <div key={message.id}
            className={`chat ${isSender ? "chat-start":"chat-end"} relative`}>
              {isReviewPage && (
                <p className="absolute -bottom-5 text-xs text-gray-300">sent {new Date(message.created_at).toLocaleString()}
                </p>
              )}
              ...
              : "chat-bubble-secondary bg-gray-200 text-gray-700" }`}>
                <ReactMarkdown 
                  ...
                  components={{
                    ...
                    p:({node,...props})=>(
                      <p {...props}
                      className={`whitespace-break-space mb-5 ${
                        message.content === "Thinking..." && "animate-pulse"} ${isSender ? "text-white" : "text-gray-700"}`}
                      />
                    ),
                    a:({node,...props})=>(
                      <a {...props}
